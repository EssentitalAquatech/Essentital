...
// ----------------------------------------------------
// 1️⃣ GET FARMERS BY AGENT (With Access Control & Photo URLs)
// ----------------------------------------------------
export const getFarmersByAgent = async (req, res) => {
  try {
    const { agentId, viewerId } = req.query;
    ...
    const allFarmers = await Farmer.find({ createdBy: agentId })
      .select("name contact village photo _id pondFiles fishFiles pondImage")
      .sort({ name: 1 });
    ...
    const farmersWithAccess = await Promise.all(
      allFarmers.map(async (farmer) => {
        ...
        const formatFile = (file) =>
          file ? `${req.protocol}://${req.get("host")}/uploads/${file}` : null;
        ...
        if (access) {
          ...
          return { 
            ...fullFarmer.toObject(),
            accessApproved: true,
            photo: formatFile(fullFarmer.photo),
            pondImage: formatFile(fullFarmer.pondImage),
            pondFiles: fullFarmer.pondFiles.map(formatFile),
            fishFiles: fullFarmer.fishFiles.map(formatFile),
          };
        }
        ...
        return {
          ...
          village: farmer.village,
          accessApproved: false,
          photo: formatFile(farmer.photo),
          pondImage: formatFile(farmer.pondImage),
          pondFiles: farmer.pondFiles.map(formatFile),
          fishFiles: farmer.fishFiles.map(formatFile),
        };
      })
    );
...
export const addFarmer = async (req, res) => {
  try {
    ...
    const missingFields = requiredFields.filter(f => !req.body[f] || req.body[f].toString().trim() === '');
    if (missingFields.length > 0) {
    ...
    const pondImage = req.files?.pondImage?.[0] ? `uploads/${req.files.pondImage[0].filename}` : "";
    ...
    if (totalPonds > 0) {
      for (let i = 1; i <= totalPonds; i++) {
        const tempPondId = `TEMP-${Date.now()}-${i}-${Math.random().toString(36).substring(2,7)}`;
        pondsArray.push({
          pondId: tempPondId,
          pondNumber: i,
          pondArea: req.body[`pondArea${i}`] || "",
    ...
    const formatFile = (file) => file ? `${req.protocol}://${req.get("host")}/${file}` : null;
    const formattedFarmer = {
      ...newFarmer.toObject(),
      photo: formatFile(newFarmer.photo),
      pondImage: formatFile(newFarmer.pondImage),
      pondFiles: newFarmer.pondFiles.map(formatFile),
      fishFiles: newFarmer.fishFiles.map(formatFile)
    };
...
export const getFarmers = async (req, res) => {
  try {
    ...
    const formattedFarmers = farmers.map(f => ({
      ...f.toObject(),
      photo: formatFile(f.photo),
      pondImage: formatFile(f.pondImage),
      pondFiles: f.pondFiles.map(formatFile),
      fishFiles: f.fishFiles.map(formatFile)
    }));
...
export const updateFarmer = async (req, res) => {
  try {
    ...
    const missingFields = requiredFields.filter(f => req.body[f] !== undefined && (!req.body[f] || req.body[f].toString().trim() === ''));
    if (missingFields.length > 0) {
      return res.status(400).json({ error: `Required fields cannot be empty: ${missingFields.join(', ')}` });
    }
    ...
    Object.keys(req.body).forEach(key => {
      if (!["userId", "ponds", "farmerId", "photo"].includes(key) && req.body[key] !== "") {
        farmer[key] = req.body[key];
      }
    });

    if (req.files?.photo?.[0]) farmer.photo = `uploads/${req.files.photo[0].filename}`;
    if (req.files?.pondImage?.[0]) farmer.pondImage = `uploads/${req.files.pondImage[0].filename}`;
    if (req.files?.pondFiles) farmer.pondFiles.push(...req.files.pondFiles.map(f => `uploads/${f.filename}`));
    if (req.files?.fishFiles) farmer.fishFiles.push(...req.files.fishFiles.map(f => `uploads/${f.filename}`));
