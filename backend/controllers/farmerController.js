  ...
  try {
    ...
    const formattedFarmer = {
      ...
      photo: newFarmer.photo
        ? `${req.protocol}://${req.get("host")}/${newFarmer.photo}`
        : null
    };
...
export const updateFarmer = async (req, res) => {
  try {
    ...
    // 🔒 SAFE FILE UPDATES - CORRECTED PATHS
    if (req.files?.photo?.[0]) {
      farmer.photo = `uploads/${req.files.photo[0].filename}`;
    }
    ...
    if (req.files?.pondFiles) {
      farmer.pondFiles.push(...req.files.pondFiles.map(f => `uploads/${f.filename}`));
    }

    if (req.files?.fishFiles) {
      farmer.fishFiles.push(...req.files.fishFiles.map(f => `uploads/${f.filename}`));
    }
...
export const addPondWithValidation = async (req, res) => {
  try {
    ...
    if (pondData.DO && isNaN(parseFloat(pondData.DO))) {
      return res.status(500).json({ error: "DO must be a number" });
    }
    ...
    // Create new pond object with corrected file paths
    const newPond = {
      pondId,
      pondNumber,
      ...pondData,
      pondImage: req.files?.pondImage?.[0] 
        ? `uploads/${req.files.pondImage[0].filename}` 
        : "",
      pondFiles: req.files?.pondFiles 
        ? req.files.pondFiles.map(f => `uploads/${f.filename}`) 
        : [],
      fishFiles: req.files?.fishFiles 
        ? req.files.fishFiles.map(f => `uploads/${f.filename}`) 
        : [],
      createdAt: new Date(),
      updatedAt: new Date()
    };
...
export const updatePondWithValidation = async (req, res) => {
  try {
    ...
    // Update pond with corrected file paths
    farmer.ponds[pondIndex] = {
      ...oldPond,
      pondId: oldPond.pondId,
      pondNumber: oldPond.pondNumber,
      ...
      pondImage: req.files?.pondImage?.[0] 
        ...
        : oldPond.pondImage,
      pondFiles: req.files?.pondFiles 
        ? [...oldPond.pondFiles, ...req.files.pondFiles.map(f => `uploads/${f.filename}`)]
        : oldPond.pondFiles,
      fishFiles: req.files?.fishFiles 
        ? [...oldPond.fishFiles, ...req.files.fishFiles.map(f => `uploads/${f.filename}`)]
        : oldPond.fishFiles,
      updatedAt: new Date()
    };
