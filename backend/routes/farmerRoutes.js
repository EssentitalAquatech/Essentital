...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const missingFields = [];
      for (const field of requiredPondFields) {
        if (!pondData[field] || String(pondData[field]).trim() === "") {
          missingFields.push(field);
        }
      }
      ...
      if (req.files?.pondImage?.[0]) {
        pondImageId = await uploadToGridFS(
          ...
          { type: 'pond-image', farmerId: farmer.farmerId },
          'pondImages'
        );
      } else {
        return res.status(400).json({ error: "Pond image is required" });
      ...
      if (req.files?.uploadSelfie?.[0]) {
        selfieId = await uploadToGridFS(
          ...
          { type: 'pond-selfie', farmerId: farmer.farmerId },
          'pondImages'
        );
      }
      ...
      if (req.files?.pondFiles) {
        pondFileIds = await uploadMultipleToGridFS(
          req.files.pondFiles,
          { type: 'pond-document', farmerId: farmer.farmerId },
          'documents'
        );
      }
      ...
      if (req.files?.fishFiles) {
        fishFileIds = await uploadMultipleToGridFS(
          req.files.fishFiles,
          { type: 'fish-document', farmerId: farmer.farmerId },
          'documents'
        );
      }
...
router.put(
  ...
  async (req, res) => {
    try {
      console.log("📝 UPDATE POND REQUEST:", req.params);
      ...
      if (Object.keys(changes).length > 0) {
        farmer.updates.push({
          snapshot: { pondId: oldPond.pondId, pondNumber: oldPond.pondNumber, ...oldPond },
          changes,
          updatedBy: updateData.userId || farmer.userId,
          createdAt: new Date()
        });
...
/* ===============================
   IMAGE ROUTE - GET IMAGE BY ID
================================ */
router.get("/image/:bucketType/:fileId", async (req, res) => {
  try {
    const { bucketType, fileId } = req.params;
    const { streamFileToResponse } = await import("../config/gridfs.js");
    await streamFileToResponse(fileId, res, bucketType);
  } catch (err) {
