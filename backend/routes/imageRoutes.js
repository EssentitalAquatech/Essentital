...
// ✅ Route 6: Get Selfie Image by Pond ID
router.get("/pond/selfie/:pondId", async (req, res) => {
  try {
    const { pondId } = req.params;
    
    console.log(`🔍 Fetching selfie for pondId: ${pondId}`);
    
    const farmer = await Farmer.findOne({ 
      "ponds.pondId": pondId 
    }).select("ponds.$");
    
    if (!farmer || !farmer.ponds?.[0]?.uploadSelfie) {
      console.log(`❌ Selfie not found for pond: ${pondId}`);
      return res.status(404).json({ error: "Selfie image not found" });
    }
    
    res.set({
      "Content-Type": "image/jpeg",
      "Content-Length": farmer.ponds[0].uploadSelfie.length,
      "Cache-Control": "public, max-age=31536000"
    });
    
    return res.send(farmer.ponds[0].uploadSelfie);
