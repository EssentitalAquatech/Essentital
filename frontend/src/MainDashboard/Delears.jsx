...
function DealersPage() {
  ...
  // ==================== IMAGE URL HELPER ====================
  // ✅ FIXED: Dealer image URL builder
  const getDealerImageUrl = (dealerId) => {
    if (!dealerId) return "/no-image.png";
    
    // Server pe ek image endpoint banao
    return `${api.defaults.baseURL}/api/dealers/${dealerId}/image?t=${Date.now()}`;
  };

  // ✅ FIXED: Convert buffer to blob URL
  const getImageFromBuffer = async (dealer) => {
    if (!dealer._id) return "/no-image.png";
    
    try {
      // Pehle check karo agar server se mil raha hai
      const response = await fetch(getDealerImageUrl(dealer._id));
      if (response.ok) {
        return getDealerImageUrl(dealer._id);
      }
      
      // Agar server endpoint nahi hai, toh local buffer use karo
      if (dealer.image && dealer.image.data) {
        // Create blob from buffer
        const blob = new Blob(
          [new Uint8Array(dealer.image.data.data || dealer.image.data)], 
          { type: dealer.image.contentType || 'image/jpeg' }
        );
      ...
      console.log("Error loading dealer image:", error);
      return "/no-image.png";
    }
  };
  ...
  const fetchDealers = async () => {
    try {
      ...
      // Agar image buffer format mein hai, toh convert karo
      const dealersWithImages = await Promise.all(
        res.data.map(async (dealer) => {
          let imageUrl = "/no-image.png";
          
          if (dealer._id) {
            imageUrl = getDealerImageUrl(dealer._id);
            
            // Agar local buffer hai toh use karo
            if (dealer.image && dealer.image.data) {
              try {
                const blobUrl = await getImageFromBuffer(dealer);
                imageUrl = blobUrl;
              } catch (error) {
                console.log("Buffer conversion error:", error);
              }
            }
          }
      ...
      setSearchResult(dealersWithImages);
    } catch (err) {
  ...
  useEffect(() => {
    return () => {
      dealers.forEach(dealer => {
        if (dealer.imageUrl && dealer.imageUrl.startsWith('blob:')) {
          URL.revokeObjectURL(dealer.imageUrl);
        }
      });
    };
  }, [dealers]);
  ...
  const handleAddDealer = async () => {
    ...
    try {
      ...
      console.log("Error adding dealer:", err);
      alert(err.response?.data?.error || t("errorAddingDealer"));
    } finally {
      setLoadingSave(false);
    }
  };
        ...
        <div className="dealers-list-container">
          {searchResult.length > 0 ? (
            searchResult.map((dealer) => (
              ...
              >
                <img
                  src={dealer.imageUrl || "/no-image.png"}
                  alt={dealer.name}
                  className="dealers-card-img"
                  loading="lazy"
                  onError={(e) => {
