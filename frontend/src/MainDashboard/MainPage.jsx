...
// Helper function for farmer image - FIXED FOR BINARY STORAGE
const getFarmerImage = (farmer) => {
  if (!farmer) return "/profile.png";
  
  // If farmer has photoUrl field (from backend)
  if (farmer.photoUrl) {
    const url = getImageUrl(farmer.photoUrl);
    console.log(`Farmer ${farmer.name} photo URL:`, url);
    return url;
  }
  ...
  if (farmer._id) {
    return getFarmerPhotoUrl(farmer._id);
  }
  
  return "/profile.png";
};

// Helper function to get pond image URL
const getPondImage = (farmerId, pond) => {
  if (!farmerId || !pond) return "/pond-placeholder.png";
  
  if (pond.pondId) {
    return getPondImageUrl(farmerId, pond.pondId);
  }
  
  return "/pond-placeholder.png";
};
...
function MainPage() {
  ...
  const fetchFarmers = async () => {
    try {
      ...
      // Ensure all farmers have photoUrl
      const farmersWithUrls = res.data.map(farmer => ({
        ...farmer,
        photoUrl: farmer.photoUrl || getFarmerPhotoUrl(farmer._id)
      }));
      
      setFarmers(farmersWithUrls || []);
    } catch (err) {
      console.log("Fetch Farmers Error:", err);
      alert("Error fetching farmers: " + err.message);
    } finally {
      setLoading(prev => ({ ...prev, fetchFarmers: false }));
    }
  };
  ...
  const addFarmer = async () => {
    ...
    // Handle farmer level files (optional)
    if (newFarmer.pondFiles && newFarmer.pondFiles.length > 0) {
    ...
    try {
      ...
      // Add the new farmer with photoUrl
      const newFarmerWithUrl = {
        ...res.data,
        photoUrl: res.data.photoUrl || getFarmerPhotoUrl(res.data._id)
      };
      
      setFarmers([...farmers, newFarmerWithUrl]);
      setShowForm(false);
      setNewFarmer(emptyFarmer);
  ...
  const updateFarmer = async () => {
    ...
    try {
      ...
      setFarmers(farmers.map(f =>
        f._id === updatedFarmer._id ? updatedFarmer : f
      ));
  ...
  const addPond = async () => {
    ...
    try {
      ...
      // Update the farmer in state
      const updatedFarmer = {
        ...res.data.farmer,
        photoUrl: res.data.farmer.photoUrl || getFarmerPhotoUrl(res.data.farmer._id)
      };
  ...
  const renderExistingFiles = (list, fileType, pondId) => {
    if (!list || list.length === 0) return null;
    ...
    return (
      <div style={{ marginTop: 6 }}>
        {list.map((file, i) => {
          ...
          if (fileType === 'pond') {
            fileUrl = getPondFileUrl(currentFarmerId, pondId || editingPondId, i);
          } else if (fileType === 'fish') {
            fileUrl = getFishFileUrl(currentFarmerId, pondId || editingPondId, i);
          }
  ...
  const handleSearch = async () => {
    ...
    try {
      const filtered = farmers.filter(f =>
        ...
        (f.name && f.name.toLowerCase().includes(searchId.toLowerCase()))
      );
            ...
            <div style={{ padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
              ...
              <div className="row g-2">
                ...
                <div className="col-md-12">
                  ...
                  {/* Show existing photo in update mode */}
                  {isUpdateMode && newFarmer.photoExisting && (
                    <div style={{ marginTop: 6 }}>
                      <p>Current Photo:</p>
                      <img
                        src={getFarmerPhotoUrl(editingFarmerId)}
                        alt="Current"
                        style={{ width: 80, height: 80, borderRadius: "50%" }}
                        onError={(e) => {
                          e.target.src = "/profile.png";
                ...
                <div className="col-md-6">
                  <label>Pond Files (Optional)</label>
                  <input
                    type="file"
                    className="form-control"
                    accept="image/*,video/*"
                    multiple
                    onChange={(e) =>
                      setNewFarmer({ ...newFarmer, pondFiles: Array.from(e.target.files) })
                    }
                    disabled={loading.addFarmer || loading.updateFarmer}
                  />
                </div>
                ...
                <div className="col-md-6">
                  <label>Fish Files (Optional)</label>
                  <input
                    type="file"
                    className="form-control"
                    accept="image/*,video/*"
                    multiple
                    onChange={(e) =>
                      setNewFarmer({ ...newFarmer, fishFiles: Array.from(e.target.files) })
                    }
                    disabled={loading.addFarmer || loading.updateFarmer}
                  />
                </div>
              ...
              <div className="modal-section">
                ...
                <div className="row g-2">
                  ...
                  <div className="col-md-6">
                    ...
                    {editingPondId && newPond.pondImageExisting && (
                      <div style={{ marginTop: 6 }}>
                        <p>Current Pond Image:</p>
                        <img
                          src={getPondImageUrl(currentFarmerId, editingPondId)}
                          alt="Current Pond"
                          style={{ width: 100, height: 100, objectFit: 'cover' }}
                          onError={(e) => {
                            e.target.src = "/pond-placeholder.png";
                            e.target.onerror = null;
                          }}
                        />
                      </div>
                  ...
                  <div className="col-md-6">
                    ...
                    {editingPondId && newPond.pondFilesExisting && renderExistingFiles(newPond.pondFilesExisting, 'pond', editingPondId)}
                  </div>
                ...
                <div className="row g-2">
                  ...
                  <div className="col-md-6">
                    ...
                    <input
                      ...
                      disabled={loading.addPond || loading.updatePond}
                    />
                    {editingPondId && newPond.fishFilesExisting && renderExistingFiles(newPond.fishFilesExisting, 'fish', editingPondId)}
                  </div>
                </div>
              </div>
