...
const History = () => {
  ...
  // ✅ Modal open hone par body ko control karna
  useEffect(() => {
    if (showOrderModal) {
  ...
  const viewOrderDetails = (order) => {
    setSelectedOrder(order);
    setShowOrderModal(true);
    // ✅ Modal open hone par body ko control
    document.body.classList.add('modal-open');
  };
  ...
  const closeOrderModal = () => {
    setShowOrderModal(false);
    setSelectedOrder(null);
    // ✅ Modal close hone par body ko control
    document.body.classList.remove('modal-open');
  };

  // ✅ ESC key se modal close karna
  useEffect(() => {
    const handleEscKey = (event) => {
      if (event.key === 'Escape' && showOrderModal) {
        closeOrderModal();
      }
    };
  ...
  return (
    <div className="history-page-container">
      ...
      {showOrderModal && selectedOrder && (
        <div className="history-order-modal-overlay" onClick={closeOrderModal}>
          <div className="history-order-modal" onClick={(e) => e.stopPropagation()}>
            <div className="history-modal-header">
              <h3>Order Details</h3>
              <button className="history-modal-close-btn" onClick={closeOrderModal}>
                ✕
